...
// Utility function to extract short bio from full bio
export const getShortBio = (fullBio: string, maxLength: number = 150): string => {
  // Remove age information from the beginning if present
  const cleanBio = fullBio.replace(/^[^(]*\(Age:\s*\d+\)\s*/, '');
  
  // Get the first sentence or first maxLength characters
  const firstSentence = cleanBio.split(/[.!?]/)[0];
  
  if (firstSentence.length <= maxLength) {
    return firstSentence.trim();
  }
  
  // If first sentence is too long, truncate at maxLength
  return cleanBio.substring(0, maxLength).trim() + '...';
}
...
export const fellows: Fellow[] = [
  // Chad Fellows
  {
    ...
    fullBio: "Zara Yasmine Hassan Ahmat Djefil (Age: 24) is a dynamic water resources engineer and climate justice advocate from Massakory, in Chad’s Hadjer-Lamis region, one of the communities at the heart of the Lake Chad crisis. With lived experience of the region’s escalating climate challenges, Zara is passionate about advancing water justice, gender equity, and community resilience. She serves as Vice President of GEDAAF/HL, where she leads grassroots efforts to empower women in climate-affected areas. As Editorial Lead at ISEC CHAD, she amplifies youth voices through media and advocacy, and recently coordinated LCOY Chad 2024, bringing together over 200 young people to shape national climate policy. Zara holds a degree in hydraulic engineering, with academic research focused on optimizing irrigation systems to improve agricultural productivity and reduce water loss in the Lake Chad Basin. She is an active contributor to regional and global youth platforms including YOUNGO and the Pan-African Climate and Gender networks and has represented Sahelian youth at high-level dialogues across Africa. Through the Lake Chad Climate Justice Fellowship, Zara seeks to deepen her impact by co-creating inclusive, youth-led solutions that address the interlinked challenges of climate change, water scarcity, and gender-based vulnerability in the Lake Chad region."
  },
  {
